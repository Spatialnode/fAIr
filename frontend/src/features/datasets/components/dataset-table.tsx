        ...
        return (
          <div className="flex items-center gap-x-2">
            <ToolTip content={title}>
              <span
                title={title ?? ""}
                className="block max-w-[200px] truncate font-semibold text-dark"
              >
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <span className="text-body-3 text-dark">
          {row.original.models_count} Model
          {row.original.models_count !== 1 ? "s" : ""}
        </span>
      ),
    },
    {
  ...
  if (isError) {
    return (
      <div className="flex flex-col items-center justify-center w-full py-20 gap-y-4">
        <p className="text-grey text-body-2base">Error loading datasets.</p>
  ...
  if (datasets.length === 0) {
    return (
      <div className="flex flex-col gap-y-4 items-center justify-center py-20">
        <NoTrainingAreaIcon />
        <p className="text-grey text-body-2base">No training dataset found.</p>
      </div>
    );
  }
